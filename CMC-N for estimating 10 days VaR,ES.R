...
sp_dat <- read_xlsx("./spx_100years.xlsx") %>% data.frame(stringsAsFactors = FALSE)
...
simulated_return_norm <- function(par, length_period){
  sam_z <- rnorm(length_period, mean=0, sd=1)
...
normal_VaR <- cbind(actual_logr10, VaR_cmc, se_VaR)
write.csv(normal_VaR, "./normal_VaR_0.99.csv")